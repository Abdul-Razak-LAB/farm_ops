...
const version = "16.2.0-canary.57";
window.next = {
    version,
    appDir: true
};
...
function appBootstrap(hydrate) {
    ...
    loadScriptsInSequence(self.__next_s, ()=>{
        ...
        if ("TURBOPACK compile-time truthy", 1) {
            ...
            if (document.cookie.includes(NEXT_INSTANT_TEST_COOKIE + '=') && typeof cookieStore !== 'undefined') {
...
function isNavigationLocked() {
    if ("TURBOPACK compile-time truthy", 1) {
        return document.cookie.includes(_approuterheaders.NEXT_INSTANT_TEST_COOKIE + '=');
    }
        ...
        case _cache.EntryStatus.Fulfilled:
            {
                ...
                if (tree.prefetchHints & _approutertypes.PrefetchHint.SubtreeHasInstant) {
                    ...
                    fetchStrategy = route.isPPREnabled ? _types.FetchStrategy.PPR : _types.FetchStrategy.LoadingBoundary;
                } else {
...
function discoverKnownRoute(now, pathname, pendingEntry, routeTree, metadataVaryPath, couldBeIntercepted, canonicalUrl, isPPREnabled, hasDynamicRewrite) {
    ...
    if (pendingEntry !== null) {
        ...
        const fulfilledEntry = (0, _cache.fulfillRouteCacheEntry)(now, pendingEntry, tree, metadataVaryPath, couldBeIntercepted, canonicalUrl, isPPREnabled);
 ...
 */ function discoverKnownRoutePart(parentKnownRoutePart, routeTree, urlPart, remainingParts, existingEntry, now, pathname, fullTree, metadataVaryPath, couldBeIntercepted, canonicalUrl, isPPREnabled, hasDynamicRewrite) {
    ...
    if (segmentAppearsInURL) {
        ...
        if (paramName === null && urlPart !== segment) {
            ...
            return (0, _cache.writeRouteIntoCache)(now, pathname, fullTree, metadataVaryPath, couldBeIntercepted, canonicalUrl, isPPREnabled);
        }
    ...
    if (existingEntry !== null) {
        ...
        entry = (0, _cache.writeRouteIntoCache)(now, pathname, fullTree, metadataVaryPath, couldBeIntercepted, canonicalUrl, isPPREnabled);
    }
    if (hasDynamicRewrite) {
...
function attemptToFulfillDynamicSegmentFromBFCache(now, segment, tree) {
    ...
    const adjustedCurrentTime = now - _navigatereducer.STATIC_STALETIME_MS + _navigatereducer.DYNAMIC_STALETIME_MS;
    const bfcacheEntry = (0, _bfcache.readFromBFCacheDuringRegularNavigation)(adjustedCurrentTime, varyPath);
    if (bfcacheEntry !== null) {
        ...
        const requestedAt = bfcacheEntry.staleAt - _navigatereducer.DYNAMIC_STALETIME_MS;
        // Now add the stale time used by dynamic prefetches.
        const dynamicPrefetchStaleAt = requestedAt + _navigatereducer.STATIC_STALETIME_MS;
...
function writeRouteIntoCache(now, pathname, tree, metadataVaryPath, couldBeIntercepted, canonicalUrl, isPPREnabled) {
    ...
    const varyPath = (0, _varypath.getRouteVaryPath)(pathname, renderedSearch, null);
    const isRevalidation = false;
...
async function fetchRouteOnCacheMiss(entry, task, key) {
    ...
    try {
        ...
        if (routeIsPPREnabled) {
            ...
            (0, _optimisticroutes.discoverKnownRoute)(Date.now(), pathname, entry, routeTree, metadataVaryPath, couldBeIntercepted, canonicalUrl, routeIsPPREnabled, false // hasDynamicRewrite
            );
        } else {
            ...
            _types.FetchStrategy.LoadingBoundary, response, serverData, entry, couldBeIntercepted, canonicalUrl, routeIsPPREnabled, headVaryParams, pathname);
        }
...
async function fetchSegmentPrefetchesUsingDynamicRequest(task, route, fetchStrategy, dynamicRequestTree, spawnedEntries) {
    ...
    try {
        ...
        if (fetchStrategy === _types.FetchStrategy.PPRRuntime) {
...
function writeDynamicTreeResponseIntoCache(now, task, fetchStrategy, response, serverData, entry, couldBeIntercepted, canonicalUrl, routeIsPPREnabled, headVaryParams, originalPathname) {
    ...
    const fulfilledEntry = (0, _optimisticroutes.discoverKnownRoute)(now, originalPathname, entry, routeTree, metadataVaryPath, couldBeIntercepted, canonicalUrl, routeIsPPREnabled, false // hasDynamicRewrite
    );
        ...
        case 0:
            {
                ...
                if (_navigatereducer.DYNAMIC_STALETIME_MS > 0) {
        ...
        case 1:
            {
                ...
                (0, _bfcache.writeToBFCache)(now, tree.varyPath, rsc, prefetchRsc, head, prefetchHead);
                if (isPage && metadataVaryPath !== null) {
                    (0, _bfcache.writeHeadToBFCache)(now, metadataVaryPath, head, prefetchHead);
                }
                return {
...
routeCacheEntry) {
    ...
    if (routeCacheEntry !== null) {
        ...
        if (metadataVaryPath !== null) {
            const now = Date.now();
            (0, _optimisticroutes.discoverKnownRoute)(now, retryUrl.pathname, null, seed.routeTree, metadataVaryPath, false, (0, _createhreffromurl.createHrefFromUrl)(retryUrl), false, true // hasDynamicRewrite
            );
        }
    }
...
routeCacheEntry) {
    ...
    if (task !== null) {
        if (freshnessPolicy !== _pprnavigations.FreshnessPolicy.Gesture) {
            (0, _pprnavigations.spawnDynamicRequests)(task, url, nextUrl, freshnessPolicy, accumulation, routeCacheEntry);
        }
...
async function navigateToUnknownRoute(now, state, url, currentUrl, currentRenderedSearch, nextUrl, currentCacheNode, currentFlightRouterState, freshnessPolicy, shouldScroll, navigateType) {
    ...
    if (metadataVaryPath !== null) {
        (0, _optimisticroutes.discoverKnownRoute)(now, url.pathname, null, navigationSeed.routeTree, metadataVaryPath, couldBeIntercepted, (0, _createhreffromurl.createHrefFromUrl)(canonicalUrl), prerendered, false // hasDynamicRewrite - not a retry, rewrite detection happens during traversal
        );
    }
    return navigateToKnownRoute(now, state, url, (0, _createhreffromurl.createHrefFromUrl)(canonicalUrl), navigationSeed, currentUrl, currentRenderedSearch, currentCacheNode, currentFlightRouterState, freshnessPolicy, nextUrl, shouldScroll, navigateType, debugInfo, // came directly from the server. If a mismatch occurs during dynamic data
...
function navigateReducer(state, action) {
    ...
    return (0, _navigation.navigate)(state, url, currentUrl, currentRenderedSearch, state.cache, state.tree, state.nextUrl, _pprnavigations.FreshnessPolicy.Default, shouldScroll, navigateType);
}
if ((typeof exports.default === 'function' || typeof exports.default === 'object' && exports.default !== null) && typeof exports.default.__esModule === 'undefined') {
    Object.defineProperty(exports.default, '__esModule', {
        value: true
...
function refreshReducer(state, action) {
    ...
    const bypassCacheInvalidation = ("TURBOPACK compile-time value", true) && action.devBypassCacheInvalidation;
...
function refreshDynamicData(state, freshnessPolicy) {
    ...
    return (0, _navigation.navigateToKnownRoute)(now, state, currentUrl, currentCanonicalUrl, refreshSeed, currentUrl, currentRenderedSearch, state.cache, currentFlightRouterState, freshnessPolicy, nextUrlForRefresh, shouldScroll, navigateType, null, // cache entry to mark as having a dynamic rewrite on mismatch. If a
...
function serverPatchReducer(state, action) {
    ...
    const now = Date.now();
    return (0, _navigation.navigateToKnownRoute)(now, state, retryUrl, retryCanonicalUrl, retrySeed, currentUrl, currentRenderedSearch, state.cache, state.tree, _pprnavigations.FreshnessPolicy.RefreshAll, retryNextUrl, shouldScroll, navigateType, null, // typically a retry after a previous mismatch, so the route was already
...
function serverActionReducer(state, action) {
    ...
    return fetchServerAction(state, nextUrl, action).then(async ({ revalidationKind, actionResult, actionFlightData: flightData, actionFlightDataRenderedSearch: flightDataRenderedSearch, redirectLocation, redirectType, isPrerender, couldBeIntercepted })=>{
        ...
        if (flightData !== undefined && flightDataRenderedSearch !== undefined) {
            ...
            if (metadataVaryPath !== null) {
                (0, _optimisticroutes.discoverKnownRoute)(now, redirectUrl.pathname, null, redirectSeed.routeTree, metadataVaryPath, couldBeIntercepted, redirectCanonicalUrl, isPrerender, false // hasDynamicRewrite
                );
            }
            return (0, _navigation.navigateToKnownRoute)(now, state, redirectUrl, redirectCanonicalUrl, redirectSeed, currentUrl, currentRenderedSearch, state.cache, currentFlightRouterState, freshnessPolicy, nextUrl, shouldScroll, navigateType, null, // have the route tree from the server response. If a mismatch occurs
        ...
        return (0, _navigation.navigate)(state, redirectUrl, currentUrl, currentRenderedSearch, state.cache, currentFlightRouterState, nextUrl, freshnessPolicy, shouldScroll, navigateType);
    }, (e)=>{
...
const _cache = __turbopack_context__.r("[project]/node_modules/next/dist/client/components/segment-cache/cache.js [app-client] (ecmascript)");
const _optimisticroutes = __turbopack_context__.r("[project]/node_modules/next/dist/client/components/segment-cache/optimistic-routes.js [app-client] (ecmascript)");
function createInitialRouterState({ navigatedAt, initialFlightData, initialCanonicalUrlParts, initialRenderedSearch, initialCouldBeIntercepted, initialPrerendered, location }) {
    ...
    if (location !== null && metadataVaryPath !== null) {
        (0, _optimisticroutes.discoverKnownRoute)(Date.now(), location.pathname, null, initialRouteTree, metadataVaryPath, initialCouldBeIntercepted, canonicalUrl, initialPrerendered, false // hasDynamicRewrite
        );
    }
...
//# sourceMappingURL=node_modules_next_dist_client_f2b19c01._.js.map